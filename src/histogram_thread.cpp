...
void c_histogram_thread::calculate_pixmap_data()
{
    ...
    if (m_colour) {
        ...
        for (int i = 0; i < 256; i++) {
            if (m_blue_table[i] > 0) {
                m_blue_table[i] = (int32_t) (((log((double) m_blue_table[i]) + 1.0)
                                              / max_value_log10)
                                             * (HISTO_HEIGHT_COLOUR / 3 - 13));
            } else {
                m_blue_table[i] = 0;
            }

            if (m_green_table[i] > 0) {
                m_green_table[i] = (int32_t) (((log((double) m_green_table[i]) + 1.0)
                                               / max_value_log10)
                                              * (HISTO_HEIGHT_COLOUR / 3 - 13));
            } else {
                m_green_table[i] = 0;
            ...
            if (m_red_table[i] > 0) {
                m_red_table[i] = (int32_t) (((log((double) m_red_table[i]) + 1.0) / max_value_log10)
                                            * (HISTO_HEIGHT_COLOUR / 3 - 13));
            } else {
                m_red_table[i] = 0;
            }
...
void c_histogram_thread::draw_histogram_pixmap(QPixmap &histogram_Pixmap)
{
    ...
    if (m_colour) {
        ...
        for (int x = 0; x < 255; x++) {
            ...
            histo_paint.drawLine(x,
                                 HISTO_HEIGHT_COLOUR - 12,
                                 x,
                                 HISTO_HEIGHT_COLOUR - 12 - m_blue_table[x]);
            histo_paint.drawLine(x,
                                 (2 * HISTO_HEIGHT_COLOUR) / 3 - 12,
                                 x,
                                 (2 * HISTO_HEIGHT_COLOUR) / 3 - 12 - m_green_table[x]);
            histo_paint.drawLine(x,
                                 HISTO_HEIGHT_COLOUR / 3 - 12,
                                 x,
                                 HISTO_HEIGHT_COLOUR / 3 - 12 - m_red_table[x]);
        }

        // Draw final column of histogram graph in red to indicate potential clipping
        histo_paint.setPen(QColor(QColor(255, 0, 0, 255)));
        histo_paint.drawLine(255,
                             HISTO_HEIGHT_COLOUR - 12,
                             255,
                             HISTO_HEIGHT_COLOUR - 12 - m_blue_table[255]);
        histo_paint.drawLine(255,
                             (2 * HISTO_HEIGHT_COLOUR) / 3 - 12,
                             255,
                             (2 * HISTO_HEIGHT_COLOUR) / 3 - 12 - m_green_table[255]);
        histo_paint.drawLine(255,
                             HISTO_HEIGHT_COLOUR / 3 - 12,
                             255,
                             HISTO_HEIGHT_COLOUR / 3 - 12 - m_red_table[255]);
    } else {
        // Monochrome
        // Create an instance of QPixmap with base histogram image on it to render the histogram graph on
